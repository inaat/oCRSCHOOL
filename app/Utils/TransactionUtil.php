...
class TransactionUtil extends Util
{
    ...
    public function getPurchaseTotals($business_id, $start_date = null, $end_date = null, $location_id = null, $user_id = null)
    {
        ...
        $opening_balance = Transaction::where('transactions.business_id', $business_id)
                    ->join('contacts AS uca', 'transactions.contact_id', 'uca.id')
                    ->whereIn('uca.type', ['supplier', 'both'])
        ...
        if ($permitted_locations != 'all') {
            ...
            $opening_balance->whereIn('transactions.location_id', $permitted_locations);
        }

        if (!empty($start_date) && !empty($end_date)) {
            $query->whereDate('transaction_date', '>=', $start_date)
                ->whereDate('transaction_date', '<=', $end_date);
            $opening_balance->whereDate('transaction_date', '>=', $start_date)
                ->whereDate('transaction_date', '<=', $end_date);
        }

        if (empty($start_date) && !empty($end_date)) {
            $query->whereDate('transaction_date', '<=', $end_date);
            $opening_balance->whereDate('transaction_date', '<=', $end_date);
        }

        //Filter by the location
        if (!empty($location_id)) {
            $query->where('transactions.location_id', $location_id);
            $opening_balance->where('transactions.location_id', $location_id);
        }

        //Filter by the location
        if (!empty($user_id)) {
            $query->where('transactions.created_by', $user_id);
            $opening_balance->where('transactions.created_by', $user_id);
        }

        $purchase_details = $query->get();
        $opening_balance_details = $opening_balance->get();
        ...
        $output['purchase_due'] = ($purchase_details->sum('final_total') -
                                    ...
                                    +$opening_balance_details->sum('final_total') 
                                    - $opening_balance_details->sum('total_paid');
;
    ...
    public function getSellTotals($business_id, $start_date = null, $end_date = null, $location_id = null, $created_by = null)
    {
       ...
       $opening_balance = Transaction::where('transactions.business_id', $business_id)
                    ->join('contacts AS uca', 'transactions.contact_id', 'uca.id')
                    ->whereIn('uca.type', ['customer', 'both'])
                    ->where('transactions.type', 'opening_balance')
                    ->where('transactions.status', 'final')
                    ->select(
                        'transactions.id',
        ...
        if (!empty($start_date) && !empty($end_date)) {
            ...
            $opening_balance->whereDate('transactions.transaction_date', '>=', $start_date)
                ->whereDate('transactions.transaction_date', '<=', $end_date);
        }

        if (empty($start_date) && !empty($end_date)) {
            $query->whereDate('transactions.transaction_date', '<=', $end_date);
            $opening_balance->whereDate('transactions.transaction_date', '<=', $end_date);
        }
        ...
        if (!empty($created_by)) {
            ...
            $opening_balance->where('transactions.created_by', $created_by);
        }
        ...
        $output['invoice_due'] = ($sell_details->sum('final_total') - $sell_details->sum('total_paid'))+$opening_balance_details->sum('final_total') - $opening_balance_details->sum('total_paid');
    ...
    */
    ...
    public function getLedgerDetails($contact_id, $start, $end)
    {
        ...
        $prev_payments_sum = $this->__paymentQuery($contact_id, $start)
            ...
            ->first();
        //Get payment totals before start date
        $prev_return_payments_sum = $this->__paymentQuery($contact_id, $start)

            ->select(
                DB::raw("SUM(IF(t.type = 'purchase_return', transaction_payments.amount, 0)) as prev_total_purchase_return_paid"),
                DB::raw("SUM(IF(t.type = 'sell_return', transaction_payments.amount, 0)) as prev_total_sell_return_paid"),
                DB::raw("SUM(IF(t.type = 'opening_balance', transaction_payments.amount, 0)) as prev_total_opening_balance_paid"))
            ->first();
       // dd($prev_return_payments_sum ->prev_total_opening_balance_paid);
        $total_prev_invoice = $previous_transaction_sums->total_purchase + $previous_transaction_sums->total_invoice ;// -  $previous_transaction_sums->total_purchase_return;
        ...
        $opening_balance =  $previous_transaction_sums->total_opening_balance;
        $opening_balance_paid = $prev_return_payments_sum->prev_total_opening_balance_paid ;
        $check_prev=$opening_balance -$opening_balance_paid;
       // dd($check_prev);
        $curr_due=0;
        foreach ($transactions as $transaction) {
            ...
            //     continue;
            // }
            $curr_due +=$transaction->final_total;
            $ledger[] = [
                'date' => $transaction->transaction_date,
                'ref_no' => in_array($transaction->type, ['sell','opening_balance', 'sell_return']) ? $transaction->invoice_no : $transaction->ref_no,
       ...
       $sell_return_sum = $transactions->where('type', 'sell_return')->sum('final_total');
        ...
        foreach ($payments as $payment) {
            ...
            $ref_no = in_array($payment->transaction_type, ['sell', 'purchase','purchase_return','sell_return', 'opening_balance']) ?  $payment->invoice_no :  $payment->ref_no;
            $note = $payment->note;
            if (!empty($ref_no)) {
            ...
            //     $note .='<small>' . __('lang_v1.custom_payment_1') . '</small>';
            //        $ledger[] = [
            ...
            $ledger[] = [
                ...
                'total' => '',
                'payment_method' => !empty($paymentTypes[$payment->method]) ? $paymentTypes[$payment->method] : '',
                'debit' =>'',// in_array($payment->transaction_type, [, 'sell_return']) ? $payment->amount : '',
                'credit' => in_array($payment->transaction_type, ['sell', 'purchase','purchase_return','sell_return', 'opening_balance']) || $payment->is_advance == 1 ? $payment->amount : '',
                'others' =>  $note ,
                'advance'=> ''
        ...
        //dd($opening_balance_due);
        $total_paid = $total_invoice_paid + $total_purchase_paid - $total_sell_return_paid - $total_purchase_return_paid;
        //$curr_due = $total_invoice + $total_purchase - $total_paid + $beginning_balance + $opening_balance_due;//-  $previous_transaction_sums->total_sell_return;
        $beginning_balance+=$previous_transaction_sums->total_sell_return +$previous_transaction_sums->total_purchase_return ;
       //$curr_due +=$prev_return_payments_sum->prev_total_sell_return_paid +$previous_transaction_sums->total_opening_balance;
        //Sort by date
      
        // if(!empty($prev_return_payments_sum->prev_total_opening_balance_paid)){
        //     $opening_balance_due+=$opening_balance_paid;
        // }
        //dd($opening_balance_due);
        if (!empty($ledger)) {
       ...
       $contact = $this->getContactInfo($contact_id);
        $opening_balance_due = $contact->opening_balance - $contact->opening_balance_paid;
        $total_due=($contact->opening_balance - $contact->opening_balance_paid)+($contact->total_invoice-$contact->invoice_received)+($contact->total_purchase-$contact->purchase_paid);
        if($opening_balance_due==0){
            $opening_balance_due+=$opening_balance_paid;
        }
        ...
        $ledger = array_merge([[
            ...
            'location' => '',
            'payment_status' => '',
            'total' => $beginning_balance+$check_prev,
            'payment_method' => '',
            'debit' => '',
            'credit' => '',
            'others' => '',
        ...
        $output = [
            ...
            'beginning_balance' => $beginning_balance+$check_prev,
            'balance_due' =>  $beginning_balance+$curr_due+$check_prev,
